...
export default function ArticleSlider({ articles }: { articles: Article[] }) {
  ...
  const prev = (e: React.MouseEvent) => { e.preventDefault(); setCurrent(c => Math.max(0, c - 1)); };
  const next = (e: React.MouseEvent) => { e.preventDefault(); setCurrent(c => c === articles.length - 1 ? 0 : c + 1); };
  ...
  return (
    ...
    >
      <Link href={`/article/${article?.slug}`} style={{ textDecoration: 'none', position: 'absolute' as const, inset: 0, display: 'block', zIndex: 1 }}>
      ...
      <div style={{ position: 'absolute' as const, bottom: 0, left: 0, right: 0, padding: isMobile ? '20px 16px' : '40px 60px', zIndex: 5, pointerEvents: 'none' }}>
        ...
        <h2 style={{ fontFamily: 'Druk, Georgia, serif', fontSize: isMobile ? '20px' : '42px', color: '#f0f5f0', lineHeight: 1.05, margin: '0 0 10px', textTransform: 'uppercase' as const, maxWidth: '800px' }}>
          {article?.titre}
        </h2>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px' }}>
          ...
          <div style={{ display: 'flex', gap: '6px', pointerEvents: 'all' }}>
            {articles.map((_, i) => (
              <div key={i} onClick={(e) => { e.preventDefault(); setCurrent(i); }} style={{ width: i === current ? '20px' : '6px', height: '6px', borderRadius: '3px', background: i === current ? '#026f5c' : 'rgba(255,255,255,0.3)', cursor: 'pointer', transition: 'all 0.3s' }} />
            ))}
          </div>
        </div>
      </div>
